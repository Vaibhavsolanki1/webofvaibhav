...
export function Entropy({ className = "", size = 400, fullscreen = false }: EntropyProps) {
  ...
  useEffect(() => {
    ...
    class Particle {
      ...
      update() {
        if (this.order) {
          ...
          if (this.x < actualSize / 2 || this.x > actualSize) this.velocity.x *= -1
          if (this.y < 0 || this.y > actualSize) this.velocity.y *= -1
          this.x = Math.max(actualSize / 2, Math.min(actualSize, this.x))
          this.y = Math.max(0, Math.min(actualSize, this.y))
        }
      }
    ...
    const gridSize = 25
    const spacing = actualSize / gridSize

    for (let i = 0; i < gridSize; i++) {
      for (let j = 0; j < gridSize; j++) {
        const x = spacing * i + spacing / 2
        const y = spacing * j + spacing / 2
        const order = x < actualSize / 2
  ...
  return (
    <div className={`${fullscreen ? 'absolute inset-0' : 'relative'} bg-black ${className}`} style={!fullscreen ? { width: size, height: size } : undefined}>
      <canvas
        ref={canvasRef}
        className={fullscreen ? "absolute inset-0 w-full h-full" : "absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2"}
      />
    </div>
  )
}
